...
(function (root, factory) {
  ...
  p5.Gen.prototype.window = function (_x, _type, _args) {
    ...
    switch (_type) {
      ...
      case "bartlett-hann":
        ...
        for (i in _x)
          _y[i] =
            a0 - a1 * Math.abs(_x[i] - 0.5) - a2 * Math.cos(2 * PI * _x[i]);
        break;
      case 4:
      ...
      case "blackman":
        ...
        for (i in _x)
          _y[i] =
            a0 - a1 * Math.cos(2 * PI * _x[i]) + a2 * Math.cos(4 * PI * _x[i]);
        break;
      ...
      case "blackman-harris":
        ...
        for (i in _x)
          _y[i] =
            a0 -
            a1 * Math.cos(2 * PI * _x[i]) +
            a2 * Math.cos(4 * PI * _x[i]) +
            a3 * Math.cos(6 * PI * _x[i]);
        break;
      ...
      case "kaiser":
        ...
        for (i in _x) {
          var above =
            PI * alpha * Math.sqrt(1.0 - (2 * _x[i] - 1.0) * (2 * _x[i] - 1.0));
          var below = PI * alpha;
      ...
      case "tukey":
        ...
        for (i in _x) {
          ...
          if (_x[i] <= ah) {
            ...
            _y[i] =
              0.5 * (1.0 + Math.cos(PI * ((2 * _x[i]) / _a - 2 / _a + 1.0)));
          }
        }
        break;
      ...
      case "slidinggaussian":
        ...
        for (var i in _x)
          _y[i] = Math.exp(
            0.0 - sq(_x[i] * 2 - 1.0 - dx) / (2.0 * sigma * sigma)
          );
        break;
        ...
        for (i in _x) {
          ...
          if (_x[i] <= _a) {
            _y[i] =
              (1.0 / _a) *
              Math.pow(
                Math.pow(_a, pwn) - Math.pow(_x[i] - _a, pwn),
                1.0 / pwn
              );
          } else {
            _y[i] =
              (1.0 / (1 - _a)) *
              Math.pow(
                Math.pow(1.0 - _a, pwn) - Math.pow(_x[i] - _a, pwn),
                1.0 / pwn
              );
          }
        }
      ...
      case "poisson":
        ...
        for (var i in _x)
          _y[i] = Math.exp(0.0 - Math.abs(_x[i] - 0.5) * (1.0 / tau));
        break;
      case "hann-poisson":
      case "poisson-hann":
      case "hannpoisson":
      case "poissonhann":
        ...
        var tau =
          25.0 *
          Math.max(_args[0] * _args[0] * _args[0] * _args[0], Number.EPSILON); // nice control
        for (i in _x) {
      ...
      case "slidingpoissonhann":
        ...
        var tau =
          25.0 *
          Math.max(_args[1] * _args[1] * _args[1] * _args[1], Number.EPSILON); // nice control
        for (i in _x) {
  ...
  p5.Ease.prototype.elasticInOut = function (_x) {
    if (_x < 0.5) {
      return (
        0.5 * Math.sin(13 * HALF_PI * (2 * _x)) * Math.pow(2, 10 * (2 * _x - 1))
      );
    } else {
      return (
        0.5 *
        (Math.sin(-13 * HALF_PI * (2 * _x - 1 + 1)) *
          Math.pow(2, -10 * (2 * _x - 1)) +
          2)
      );
    }
  };
  ...
  p5.Ease.prototype.doubleEllipticOgee = function (_x, _a, _b) {
    ...
    if (_x <= _a) {
      ...
      _y =
        1.0 - ((1.0 - _b) / (1.0 - _a)) * Math.sqrt(sq(1.0 - _a) - sq(_x - _a));
    }
  ...
  p5.Ease.prototype.doubleSquircularSigmoid = function (_x, _a, _n) {
    ...
    if (_x <= _a) {
      ...
      _y =
        _a +
        Math.pow(Math.pow(1.0 - _a, pwn) - Math.pow(_x - 1.0, pwn), 1.0 / pwn);
    }
  ...
  p5.Ease.prototype.doubleEllipticSigmoid = function (_x, _a, _b) {
    ...
    if (_x <= _a) {
      ...
      if (_a >= 1) {
        ...
        _y =
          _b +
          ((1.0 - _b) / (1.0 - _a)) * Math.sqrt(sq(1.0 - _a) - sq(_x - 1.0));
      }
    }
  ...
  p5.Ease.prototype.normalizedErf = function (_x) {
    ...
    var _y = Math.sqrt(
      1.0 - Math.exp(0 - z2 * ((a * z2 + 4.0 / PI) / (a * z2 + 1.0)))
    );
    if (z < 0.0) _y = 0 - _y;
  ...
  p5.Ease.prototype.circularFillet = function (_x, _a, _b, _c) {
    ...
    computefillet: {
      ...
      var arcAngle = 0.0;
      var dd = Math.sqrt(
        (gv1x * gv1x + gv1y * gv1y) * (gv2x * gv2x + gv2y * gv2y)
      );
      if (dd != 0.0) {
  ...
  p5.Ease.prototype.circularArcThroughAPoint = function (_x, _a, _b) {
    ...
    function isPerpendicular(pt1x, pt1y, pt2x, pt2y, pt3x, pt3y) {
      ...
      if (
        Math.abs(xDelta_a) <= Number.EPSILON &&
        Math.abs(yDelta_b) <= Number.EPSILON
      ) {
        return false;
      }
  ...
  p5.Ease.prototype.doubleSquircularOgee = function (_x, _a, _n) {
    ...
    if (_x <= _a) {
      ...
      _y =
        1.0 -
        Math.pow(Math.pow(1 - _a, pown) - Math.pow(_x - _a, pown), 1.0 / pown);
    }
